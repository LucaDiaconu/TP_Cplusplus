...
Bird::Bird(string name, string color, double wingspan, string diet, string habitat, string sounds, bool isPet, int age, double weight, double height)
        : Animal("Bird", name, color, diet, habitat, sounds, isPet, age, weight, height), wingspan(wingspan) {

}

Bird::Bird(const Bird &bird)
        : Animal(bird.getType(),  bird.getName(), bird.getColor(), bird.getDiet(), bird.getHabitat(), bird.getSounds(), bird.getIsPet(), bird.getAge(),
